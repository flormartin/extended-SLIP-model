...
for k = N_phase+1:N
    ...
    k1 = mode1(X(:,k), phi0, offset, U(j));
    k2 = mode1(X(:,k) + 0.5 * h_st * k1, phi0, offset, U(j));
    k3 = mode1(X(:,k) + 0.5 * h_st * k2, phi0, offset, U(j));
    k4 = mode1(X(:,k) + h_st * k3, phi0, offset, U(j));
