...
x_lo_knee = (X(1,N+1) - offset) + l_u * sin(X(3,N+1)); % lift off
y_lo_knee = X(2,N+1) - l_u * cos(X(3,N+1)) ;

opti.subject_to(sqrt(x_lo_knee^2 + y_lo_knee^2) == l_l);

y_td_knee = X(2,N_phase+1) - l_u * cos(X(3,N_phase+1)); % touch down    
...
opti.subject_to(X(2,:) >= 0.4);
opti.subject_to(X(1,1) == 0.0);
opti.subject_to(X(5,:) >= 0.0);
opti.subject_to(X(3,:) >= -pi/4);
opti.subject_to(X(3,:) <= pi/4);

opti.subject_to( X(2,:) - l_u * cos(X(3,:)) >= 0 ); % - l_l * cos(X(3,1:N_phase)+X(4,1:N_phase))
...
% x_foot = zeros(1,N);
% y_foot = zeros(1,N);
% x_foot(1:N_phase) = x_knee(1:N_phase) + l_l * sin(angle_sol(1:N_phase));
% y_foot(1:N_phase) = y_knee(1:N_phase) - l_l * cos(angle_sol(1:N_phase));
% 
% % Foot - stance
% x_foot(N_phase+1:N) = offset_sol * ones(1,N_phase);
% y_foot(N_phase+1:N) = zeros(1,N_phase);
% 
figure; hold on;
for i = 1:N
...
%     plot(x_hip_sol(i), y_hip_sol(i),'o','Color','k');
%     plot(x_knee(i), y_knee(i),'o','Color','b');
% %     plot(x_knee(i), phi_sol(i),'o','Color','b');
% end
