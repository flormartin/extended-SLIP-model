...
% opti.subject_to(theta_sol(N+1) == 0);
...
% Knee_2 = X(2,N+1) - l1 * cos(X(3,N+1));
% Knee_1 = X(1,N+1) + l1 * sin(X(3,N+1));
% offset_1 = X(1,N_phase+1) + L0 * sin(X(3,N_phase+1));
% angle_1 = atan(( offset_1 - Knee_1/Knee_2 ));
% theta_1 = X(3,N+1) - angle_1;
% opti.subject_to(theta_1 == 0);
...
opti.subject_to(X(2,N+1) == L0 * cos(X(3,N+1)));
...
opti.subject_to(X(1,N+1) <= 4); % minimum length of step
...
% load('relaxed_sol.mat')
% % x_sol1 = zeros(6,N+1);
% x_sol1 = x_sol;
% % x_sol1(2:6,1) = x_sol(2:6,N_phase+1);
% xe = [1.3726, 0.9680, 0.2538, 2.5796, -2.5684, 0.7995];
% x0 = xe';
% x_sol1(2:6,1) = x0(2:6);
% 
% opti.set_initial(X, x_sol1);
...
% opti.set_initial(phi0, 0);
% opti.set_initial(alpha0, 0);
% opti.set_initial(omega, 0);

% for solution with guard
% load('one_solution.mat')
% load('another_solution.mat')
load('strict.mat')
